...
<div {{ $attributes }} x-data="{ open: false }" @keydown.escape.window="open = false" :class="{ 'z-40': open }">
    ...
    <template x-teleport="body">
        <div class="fixed left-0 top-0 z-[99] flex h-screen w-screen items-center justify-center" x-show="open" x-cloak>
            {{-- Background blur --}}
            <div class="absolute inset-0 h-full w-full cursor-pointer bg-white bg-opacity-70 backdrop-blur-sm"
            ...
            <div class="relative w-full border border-neutral-200 bg-white px-7 py-6 shadow-lg sm:max-w-lg sm:rounded-lg"
                ...
                <div class="flex items-center justify-between pb-3">
                    ...
                    {{-- Close button --}}
                    <button class="h-8 w-8 rounded-full text-gray-600 hover:bg-gray-50 hover:text-gray-800"
                        @click="open=false">
                        <x-fas-xmark class="m-auto h-5 w-5" />
                    </button>
                </div>

                <div {{ $body->attributes->class(['relative', 'w-auto', 'pb-8']) }}>
                    {{ $body }}
                </div>
                <div
