...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('requests', function (Blueprint $table) {
            ...
            $table->foreign(['dropoff_id'], 'requests_ibfk_4')->references(['id'])->on('addresses')->onDelete('CASCADE');
            $table->foreign(['ride_id'], 'requests_ibfk_1')->references(['id'])->on('rides')->onDelete('CASCADE');
            $table->foreign(['pickup_id'], 'requests_ibfk_3')->references(['id'])->on('addresses')->onDelete('CASCADE');
        });
    }
