...
class Controller {
  ...
  public function signIn() {
    ...
    $sources_used = "https://getbootstrap.com/docs/5.0/examples/sign-in/";
    $title = "Sign in";
    $styles = array("largeheader", "signin");
    $vars = array("sources_used" => $sources_used, "title" => $title, "styles" => $styles);
    if (isset($error_msg)) {
  ...
  public function newRide() {
    ...
    if (isset($_POST["start-time"])) {
      ...
      $ride_insert = $this->db->query("insert into ride (driver_email, start_time, orig_addr, dest_addr, seats_total, description) values (?, ?, ?, ?, ?, ?);", "ssssis", $_SESSION["email"], $_POST["start-time"], $_POST["orig-addr"], $_POST["dest-addr"], $_POST["seats"], $_POST["description"]);
  ...
  public function profile($error = null) {
    ...
    if (isset($_POST["year"])) {
      ...
      if (!empty($_POST["year"])) $update = $this->db->query("update user set year = ? where email = ?;", "is", $_POST["year"], $_SESSION["email"]);
      if (!empty($_POST["major"])) $update = $this->db->query("update user set major = ? where email = ?;", "ss", $_POST["major"], $_SESSION["email"]);
      if (!empty($_POST["bio"])) $update = $this->db->query("update user set bio = ? where email = ?;", "ss", $_POST["bio"], $_SESSION["email"]);
      ...
      if (!empty($_POST["contact-phone"]) || !empty($_POST["contact-first-name"]) || !empty($_POST["contact-last-name"]) || !empty($_POST["contact-relationship"])) {
        if (!(!empty($_POST["contact-phone"]) && !empty($_POST["contact-first-name"]) && !empty($_POST["contact-last-name"]) && !empty($_POST["contact-relationship"]))) {
          ...
          $insert_or_update = $this->db->query("INSERT INTO user_emergency_contact (user_email, phone, first_name, last_name, relationship) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE first_name = VALUES(first_name), last_name = VALUES(last_name), relationship = VALUES(relationship)", "sssss", $_SESSION["email"], $_POST["contact-phone"], $_POST["contact-first-name"], $_POST["contact-last-name"], $_POST["contact-relationship"]);
        }
      }
      ...
      if (!empty($_POST["car-license-plate"]) || !empty($_POST["car-make"]) || !empty($_POST["car-color"])) {
        if (!(!empty($_POST["car-license-plate"]) && !empty($_POST["car-make"]) && !empty($_POST["car-color"]))) {
          ...
          $select = $this->db->query("select car_license_plate from driver where email = ?;", "s", $_SESSION["email"]);
          if (!empty($select)) {
      ...
      if (!empty($_POST["contributions"])) $insert_or_update = $this->db->query("INSERT INTO rider (email, contributions) VALUES (?, ?) ON DUPLICATE KEY UPDATE contributions = VALUES(contributions)", "ss", $_SESSION["email"], $_POST["contributions"]);
    }

    // Get all user info
    $user_info = $this->db->query("select * from user where email = ?;", "s", $_SESSION["email"]);

    $contact_query = $this->db->query("select * from user_emergency_contact where user_email = ?;", "s", $_SESSION["email"]);
    if (!empty($contact_query)) $contact_info = $contact_query;
    else $contact_info = array(array("phone" => "", "first_name" => "", "last_name" => "", "relationship" => ""));

    $car_query = $this->db->query("select * from driver_car where license_plate = (select car_license_plate from driver where email = ?);", "s", $_SESSION["email"]);
    if (!empty($car_query)) $car_info = $car_query;
    else $car_info = array(array("license_plate" => "", "make" => "", "color" => ""));

    $rider_query = $this->db->query("select contributions from rider where email = ?;", "s", $_SESSION["email"]);
  ...
  public function myRides() {
    ...
    $joined = array();
    $joined_entries = $this->db->query("select id from ride_riders where rider_email = ?;", "s", $_SESSION["email"]);
  ...
  public function deleteRide() {
    if (!empty($this->get_vars) and array_key_exists("id", $this->get_vars)) {
      ...
      } else if ($ride[0]["driver_email"] != $_SESSION["email"]) {
  ...
  public function leaveRide() {
    if (!empty($this->get_vars) and array_key_exists("id", $this->get_vars)) {
      ...
      if ($ride === false) {
        ...
        $entry = $this->db->query("select * from ride_riders where id = ? and rider_email = ?;", "is", $id, $_SESSION["email"]);
        if ($entry === false) {
          $error_msg = "Error checking for entry.";
        } else if (empty($entry)) {
          $error_msg = "No entry found with given ride ID and rider email.";
        } else {
          $delete = $this->db->query("delete from ride_riders where id = ? and rider_email = ?;", "is", $id, $_SESSION["email"]);
  ...
  public function request() {
    ...
    if (!empty($this->get_vars) and array_key_exists("id", $this->get_vars)) {
      ...
      $request = $this->db->query("select * from request where id = ? and rider_email = ?;", "is", $id, $_SESSION["email"]);
      if (!empty($request)) {
  ...
  public function getNotifs() {
    ...
    $user_rides = $this->db->query("select id from ride where driver_email = ?;", "s", $_SESSION["email"]);
    if ($user_rides === false) {
    ...
    $db_responses = $this->db->query("select * from response where rider_email = ?;", "s", $_SESSION["email"]);
  ...
  public function deleteResponse() {
    if (!empty($this->get_vars) and array_key_exists("ride", $this->get_vars)) {
      ...
      $response = $this->db->query("select * from response where id = ? and rider_email = ?;", "is", $id, $_SESSION["email"]);
      if ($response === false) {
      ...
      $delete = $this->db->query("delete from response where id = ? and rider_email = ?;", "is", $id, $_SESSION["email"]);
      if ($delete === false) {
  ...
  public function report() {
    if (isset($_POST["reportee-email"])) {
      ...
      if ($reportee === false) {
        ...
        $insert = $this->db->query("insert into report (reporter_email, reportee_email, reason, info) values (?, ?, ?, ?);", "ssss", $reporter_email, $reportee_email, $_POST["reason"], $_POST["info"]);
        if ($insert === false) {
